...
[/IOTCONNECT OTA guide](https://dev-docs.iotconnect.io/c2d-ota/), and the
[OTA video walkthrough](https://www.youtube.com/watch?v=muHbYq0TfYI) shows
the whole portal flow in a few minutes — recommended before your first
push. The concrete flow for these demos:
...
2. **Open the Firmware section** — in the portal it lives in the
   **Device** module's toolbar:

   <img src="boards/media/iotconnect/ota-firmware-nav.png" alt="Device module toolbar with the Firmware entry" width="700"/>

   The Firmware page lists your firmwares and carries the two buttons the
   rest of the flow uses — **Create Firmware** and **OTA Updates**:

   <img src="boards/media/iotconnect/ota-firmware-page.png" alt="Firmware page with Create Firmware and OTA Updates buttons" width="700"/>

3. **Create the firmware entry** — **Create Firmware** opens the hardware
   version form: name it, select your device's **template**, and set
   hardware version `1.0` (the software version field can carry the first
   payload version):

   <img src="boards/media/iotconnect/ota-create-firmware.png" alt="Create hardware version form" width="700"/>

4. **Upload the software version** — from the firmware's **Software
   Version** page, add the `.signed.bin` as software version `1.0.1`. The
   version must match the payload's signed image version, and a version
   must be **released** (not draft) before it can be pushed:

   <img src="boards/media/iotconnect/ota-software-version.png" alt="Software version list with a draft upload" width="700"/>

5. **Push** — **OTA Updates** brings up the release form: pick the
   hardware version, software version, and template, choose the target (a
   device, a group, or everything on the template), then **Update** — or
   schedule it for later. The **OTA Update History** below tracks every
   push:

   <img src="boards/media/iotconnect/ota-updates-push.png" alt="OTA Updates push form and update history" width="700"/>

6. **Watch it land** — the device console logs every stage (below) and
   telemetry's `sys.fw` reports the new version after the swap. Note that
   the history can show *Successful* as soon as the device acknowledges
   the download — the authoritative end-to-end confirmation is the
   device's `sys.fw` value changing, which only happens after MCUboot has
   swapped and the new image has confirmed itself.
