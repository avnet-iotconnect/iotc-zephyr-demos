...
LOG_MODULE_REGISTER(model, CONFIG_APP_LOG_LEVEL);
#endif /* CONFIG_LOG */
...
constexpr int kTensorArenaSize = CONFIG_APP_TENSOR_ARENA_SIZE_KB * 1024;

#if DT_HAS_CHOSEN(zephyr_arena)
    #define ZEPHYR_ARENA_SECTION \
        Z_GENERIC_SECTION(LINKER_DT_NODE_REGION_NAME(DT_CHOSEN(zephyr_arena)))
#else
    #define ZEPHYR_ARENA_SECTION
#endif /* DT_HAS_CHOSEN(zephyr_arena) */

// An area of memory to use for input, output, and intermediate arrays.
// (Can be adjusted based on the model needs.)
    static uint8_t s_tensorArena[kTensorArenaSize] __ALIGNED(16)
        ZEPHYR_ARENA_SECTION;
...
uint8_t* GetTensorData(TfLiteTensor* tensor, tensor_dims_t* dims, tensor_type_t* type)
{
    ...
    LOG_INF("tensor: rank %d bytes %u tflite-type %d dims %d %d %d %d",
            tensor->dims ? tensor->dims->size : -1, (unsigned)tensor->bytes,
            (int)tensor->type,
            tensor->dims && tensor->dims->size > 0 ? tensor->dims->data[0] : -1,
            tensor->dims && tensor->dims->size > 1 ? tensor->dims->data[1] : -1,
            tensor->dims && tensor->dims->size > 2 ? tensor->dims->data[2] : -1,
            tensor->dims && tensor->dims->size > 3 ? tensor->dims->data[3] : -1);
