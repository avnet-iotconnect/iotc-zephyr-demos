...
{
	LOG_DBG("Found boxes count %d\r\n",retCnt);
	for (int i=0; i<retCnt; i++,p++) {
		LOG_DBG("%0.2f%%, x1: %d, y1: %d, x2: %d, y2: %d\r\n",
...
void init_face_det(int originalWidth, int originalHeight)
{
	...
	LOG_INF("model input tensor: %dx%dx%d (stitch %dx%dx%d)",
		(int)s_inputDims.data[1], (int)s_inputDims.data[2],
		(int)s_inputDims.data[3], MODEL_IN_H, MODEL_IN_W, MODEL_IN_C);
...
#if DT_HAS_CHOSEN(zephyr_modelbuf)
	/* Copy the stitched camera image (MODEL_IN_W x MODEL_IN_H, from the
	 * slice copies) into the model's input tensor. The tensor dims come
	 * from the loaded model and may differ from the stitch geometry (the
	 * original code assumed the NPU model's 160x160 input and overran the
	 * arena for smaller inputs) -- center-crop/center-fit row by row and
	 * never write outside the tensor.
	 */
	const int in_h = s_inputDims.data[1];
	const int in_w = s_inputDims.data[2];
	const int in_c = s_inputDims.data[3];

	memset(s_inputData, 0, (size_t)in_h * in_w * in_c);
	if (in_c == MODEL_IN_C) {
		const int copy_w = MIN(in_w, MODEL_IN_W);
		const int copy_h = MIN(in_h, MODEL_IN_H);
		const int src_x0 = (MODEL_IN_W - copy_w) / 2;
		const int src_y0 = (MODEL_IN_H - copy_h) / 2;
		const int dst_x0 = (in_w - copy_w) / 2;
		const int dst_y0 = (in_h - copy_h) / 2;

		for (int y = 0; y < copy_h; y++) {
			memcpy(s_inputData +
				       ((size_t)(dst_y0 + y) * in_w + dst_x0) * in_c,
			       model_input_buf +
				       ((size_t)(src_y0 + y) * MODEL_IN_W + src_x0) *
					       MODEL_IN_C,
			       (size_t)copy_w * in_c);
		}
	} else {
		LOG_ERR("model input channels %d unsupported (stitch is %d)",
			in_c, MODEL_IN_C);
	}
#endif /* DT_HAS_CHOSEN(zephyr_modelbuf) */
